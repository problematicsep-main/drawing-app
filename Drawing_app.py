...
class FreeHandDrawing(tk.Tk):
    ...
    def _create_widgets(self):
        ...
        colorList = ttk.Combobox(topframe, textvariable=self.col_select,
                                 values=['Black', 'Green', 'Brown', 'Red', 'Yellow'], state='readonly', width=10)
        colorList.current(0)
        ...
        tk.Button(topframe, text="Undo", bg='blue',
                  fg='white', activebackground='blue4',
                  activeforeground='white', command=self._undo).grid(row=0, column=2, padx=5)

        tk.Button(topframe, text='Clear', bg='brown',
                  fg='white', activebackground='brown4', activeforeground='white',
    ...
    def _on_movement(self, event):
        ...
        if self._xold is not None and self._yold is not None:
            ...
            self.canvas.create_line(self._xold, self._yold, event.x, event.y,
                                    smooth=True, width=2*self.thickness, fill=self.color, tag=tag)
